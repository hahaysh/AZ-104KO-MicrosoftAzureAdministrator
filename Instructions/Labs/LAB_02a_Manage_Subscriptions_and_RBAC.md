...
1. 관리 그룹 목록에서 새로 만든 관리 그룹을 나타내는 항목을 클릭한 다음 해당 **details**를 표시합니다.
...
1. Azure Portal에서 **az104-02-mg1** 관리 그룹으로 돌아가서 해당 **details** 을 표시합니다.

1. **액세스 제어(IAM)** 를 클릭하고 **역할 할당** 다음에 **+ 추가 - 역할 할당 추가**를 클릭하고 새로 만든 사용자 계정에 **Support Request Contributor (Custom)** 역할을 할당합니다.
...
1. **InPrivate** 브라우저 창에 있는 **도움말+지원 | 새 지원 요청** 블레이드의 **기본 사항** 탭에서 "문제 유형 선택"중에서 **서비스 및 구독 제한(할당량)** 선택하고 이 랩에서 사용 중인 구독이 **Subscription** 드롭다운 목록에 나열되어 있는지 확인합니다.

    >**참고**: **Subscription** 드롭다운 목록에 이 랩에서 사용하고 있는 구독이 나타날 경우, 이는 사용 중인 계정에 구독별 지원 요청을 위해 필요한 사용 권한이 있음을 나타냅니다.
...
1. Azure Portal에서 **az104-02-mg1** 관리 그룹으로 이동하여 해당 **details**을 표시합니다.

1. Azure 구독 오른쪽 끝에 **줄임표** 아이콘을 마우스 오른쪽 단추로 클릭하고 **이동**을 클릭합니다.
