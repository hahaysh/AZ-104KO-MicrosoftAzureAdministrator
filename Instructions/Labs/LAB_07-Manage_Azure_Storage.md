...
1. **licenses/LICENSE** 블레이드에서 사용 가능한 옵션을 검토합니다. 
    ...
    > **참고**: Blob 다운로드, 액세스 계층 변경(현재 **핫**으로 설정), 임대 취득(현재 **잠금 해제됨**인 임대 상태를 **잠금**으로 변경) 옵션이 있으며 Blob을 수정하거나 삭제할 수 없도록 보호할 수 있습니다. 또한 사용자 지정 메타데이터를 할당할 수도 있습니다(임의 키와 값 쌍 지정). 파일을 다운로드하지 않고 Azure Portal 인터페이스에서 직접 **수정**할 수도 있습니다. 스냅샷을 만들고 SAS 토큰을 생성할 수도 있습니다(다음 작업에서 이 옵션을 살펴봅니다). 
